...
const create_client = function (token, config) {
  ...
  metrics.send_request = function (options, callback) {
    ...
    const { key } = metrics.config;
    const { secret } = metrics.config;
  ...
  const send_batch_requests = function (options, callback) {
    ...
    const event_batches = chunk(event_list, max_batch_size);
    const request_batches = max_concurrent_requests
      ? chunk(event_batches, max_concurrent_requests)
      : [event_batches];
    const total_event_batches = event_batches.length;
    const total_request_batches = request_batches.length;
    ...
    function send_next_request_batch(index) {
      ...
      const cb = function (errors, results) {
        index += 1;
